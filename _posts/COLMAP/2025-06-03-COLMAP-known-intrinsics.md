...
이제 이 수식에서 카메라 좌표계의 원점, 즉 $x=0$를 넣으면?

$$
X =R^t(0-T) = -R^t \cdot T
$$

이것이 바로 카메라의 월드 좌표계에서의 위치입니다.

#### 🔍 해석: $x=0$이란?
$x=0$이란 “카메라 좌표계에서 원점을 의미” → 즉, 카메라의 중심점

우리가 알고 싶은 것은 이 카메라 중심이 월드 좌표계에서는 어디에 있는가?

따라서 이 위치는 $x=0$일 때 $X$가 무엇인지로 정의할 수 있음

📌 카메라의 위치(Position) 계산
카메라의 위치는, 월드 좌표계에서 카메라 원점이 어디에 있는가를 뜻하며, 위 수식에서 $x=0$인 경우로 계산됩니다.
