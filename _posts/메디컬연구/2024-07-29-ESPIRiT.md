...
- 같은 말로 행렬 A의 column vector들이 선형 종속인 경우, det(A)=0이며, $A\bold{x}=\bold{0}$의 해가 존재합니다.
- det(A)=0이면, 행렬 A는 Rank가 완전하지 않은 행렬이어서, $A\bold{x}=\bold{0}$의 해가 반드시 존재하게 되며, 이 해들은 Null space를 형성합니다.
