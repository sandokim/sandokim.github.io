...
***calibration data의 Null space는 배제하고, Row space(=linearly independent한 해집합)에서 span하는 subspace에서 벡터들의 선형조합으로 unacquired k-space value를 extraploate하여 찾겠다는 의미입니다.***
...
- ..., ***ESPIRiT calibration has been performed and the accuracy of the obtained sensitivity maps has been evaluated by projecting fully-sampled coil images*** $m_i$ ***of the original data onto the subspace spanned by the maps.***
  - ***sensitivity maps에 의해 spanned 된 subspace 상으로 fully-sampled coil images를 projection한 것을 reference로 하여 ESPIRiT으로부터 얻은 sensitivity maps의 성능을 평가하였습니다.*** (매우 중요한 문장)
