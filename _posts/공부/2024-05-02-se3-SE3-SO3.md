...
`deltaT`는 SE(3) 그룹에 속하는 4x4 변환 행렬로, 회전과 이동을 결합한 리지드 바디 변환을 나타냅니다. 구조는 다음과 같습니다:
...
여기서 $R$은 3x3 회전 행렬이며 SO(3) 그룹에 속합니다. $t$는 3x1 이동 벡터입니다. 이 행렬을 사용하여 3D 공간에서의 객체 위치를 변환할 수 있습니다. `deltaT`는 각속도와 선속도를 이용하여 이러한 변환을 계산합니다.
