...
**코드: Lie Algebra인 se(3)는 linear space 이고, Lie Group인 SE(3)에 존재하는 constraints가 없기 때문에 Lie Algebra인 se(3)에 대해서 미분을 계산하고
최종적으로 다시 Lie Group인 SE(3)로 변환하는 과정을 거쳐 강체의 회전과 이동을 모두 표현할 수 있습니다.**
