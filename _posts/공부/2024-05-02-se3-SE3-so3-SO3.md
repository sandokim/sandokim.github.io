...
***정리하면 se(3)는 회전인 so(3)를 포함하는 관계이고, SE(3)는 회전인 SO(3)를 포함하는 관계이므로, se(3)와 SE(3)의 관계로 so(3)와 SO(3)의 순수한 회전에 추가적으로 이동 벡터를 표현할 수 있습니다.***

**코드: Li Algebra인 se(3)는 linear space 이고, Li Group인 SE(3)에 존재하는 constraints가 없기 때문에 Li Algebra인 se(3)에 대해서 미분을 계산하고
최종적으로 다시 Li Group SE(3)로 변환하는 과정을 거칩니다.**
