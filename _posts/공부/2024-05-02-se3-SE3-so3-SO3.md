...
코드: Li Algebra se(3), so(3)는 linear space 이고, Li Group SO(3), SE(3)에 존재하는 constraints가 없기 때문에 Li Algebra인 se(3), so(3)에 대해서 미분을 계산하고
최종적으로 다시 Li Group SO(3), SE(3)로 변환하는 과정을 거친다.

따라서 3DGS에서도 se(3)에서 미분값을 계산하고 se3_to_SE3 함수로 최종적으로는 SE(3)로 반환하는 것을 확인할 수 있다.
